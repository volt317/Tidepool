// web/src/App.tsx — Tidepool distro survey console (typed against shared/types).
//
...
type Filters = { advisories: boolean; drift: boolean };
...
const api = async <T,>(path: string, opts?: RequestInit): Promise<{ status: number; body: T & ApiExtra }> => {
  ...
  const body = (await res.json().catch(() => ({}))) as T & ApiExtra;
...
function Mark({ status, title }: { status: string; title?: string }) {
...
function SourceHealth({ sources }: { sources: SourceRecord[] }) {
        ...
        <div className="grid" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 10, marginTop: 10 }}>
          {sources.map((s) => (
            <div key={s.id} style={{ background: C.shallows, border: `1px solid ${C.poolEdge}`, borderRadius: 6, padding: 10 }}>
              ...
              {s.signedBy && s.signedBy.length > 0 && (
                <div style={{ fontSize: 11.5, color: C.tide, marginTop: 3 }}>✓ signed by {s.signedBy.join("; ")}</div>
              )}
...
function EnrichPanel({ record }: { record: EnrichRecord }) {
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
                <span style={{ ...mono, fontSize: 12, color: C.mist }}>
                  {current.status === "ready"
                    ? `${current.packageCount.toLocaleString()} packages · synced ${fmtDate(current.finishedAt)} ${current.finishedAt ? new Date(current.finishedAt).toLocaleTimeString() : ""}`
                    : current.status}
                </span>
