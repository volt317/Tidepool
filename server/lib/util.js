...
export async function timedFetch(url, opts = {}, ms = 60000) {
...
export async function fetchBytes(url, opts, ms) {
...
export const gunzip = (buf) => gunzipSync(buf);
...
export function inReleaseSha256Table(text) {
  const table = {};
  let inTable = false;
  for (const line of text.split("\n")) {
...
export function splitDebVersion(v) {
  let epoch = 0, rest = v;
...
export class DiskCache {
  ...
  get(key, ttlMs) {
    ...
    try {
      const wrapped = JSON.parse(readFileSync(p, "utf8"));
